...
namespace SalesSahayak.Components
{
    public partial class ShowEnquiryDetails : System.Web.UI.Page
    {
        ...
        {
            ll = Request.QueryString["Eq_id"];
            prefix = ll.Substring(0, 3);

            i = Convert.ToInt32(ll.Substring(3, 4));
            j = Convert.ToInt32(ll.Substring(7, 2));
        ...
        void updatefollowup(int result, string followuptext)
        {
            ...
            {
                string query = "Update Enquiry set Follow_Up='" + "Not Done" + "' where Enquiry_Id='" + ll + "'";
        ...
        {
            string query = "SELECT Follow_Up FROM Enquiry where Enquiry_Id='" + ll + "'";
            con = new OleDbConnection(connectionString);
            con.Open();
            cmd = new OleDbCommand(query, con);
        ...
        void BindSingleData()
        {
            ...
            if (myreader.Read())
            {
                ...
                status.Text = myreader[3].ToString();
                followuptext = myreader[4].ToString();
                Followup.Text = followuptext;
                followfetcheddate = myreader[5].ToString();
            }
            con.Close();
        ...
        protected void FollowUp_Click(object sender, EventArgs e)
        {
            ...
            string query = "Update Enquiry set Follow_Up='" + "Done" + "',FollowUpDate='" + date + "' where Enquiry_Id='" + ll + "'";
        ...
        {
            string query = "Update Enquiry set Status='" + "Closed" + "' where Enquiry_Id='" + ll + "'";
            OleDbConnection con = new OleDbConnection(connectionString);
            con.Open();
            OleDbCommand cmd = new OleDbCommand(query, con);
            cmd.ExecuteNonQuery();
