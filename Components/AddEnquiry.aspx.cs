...
namespace SalesSahayak.Components
{
    public partial class AddEnquiry : System.Web.UI.Page
    {
        ...
        string fetchedEnquiryId; //!--- To Store Fetched Enquiry Id Temperary
        ...
        //!--- Function For Fetching the Last or Max Enquiry Id in Database
        void fetchenquiryid()
        {
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                ...
                conn.Open();
                cmd1.CommandText = "SELECT MAX(Enquiry_Id) FROM Enquiry WHERE Enquiry_Type = ? AND Company_Id ="+Com_Id;
                cmd1.Parameters.AddWithValue("@Enquiry_Type", "MAIN");
                
                OleDbDataReader reader1 = cmd1.ExecuteReader();
                if (reader1.Read() && !reader1.IsDBNull(0)) 
                {
                    fetchedEnquiryId = reader1[0].ToString();
                    prefix = fetchedEnquiryId.Substring(0, 3);
                    i = Convert.ToInt32(fetchedEnquiryId.Substring(3, 4));
                    j = Convert.ToInt32(fetchedEnquiryId.Substring(7, 2));
                    reader1.Close();
       ...
       int getTotalEnquiryValue()
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            Com_Id = Session["Com_Id"].ToString();
                ...
                fetchenquiryid(); //!--- Calling the fetchEnquiryId to get Last or Max Enquiry Id

                i++;  //!--- Increaing the Main Enquiry Id by one
                    ...
                    for (int z = 1; z < ControlCounter; z++)
                    {
                        ...
                        if (ddlProduct != null && tbQuanity != null)
                        {
                            ...
                            if (j == 0)
                            {
                                ...
                                var date = dt.Date;
                                cmd.CommandText = "Insert into Enquiry Values('" + enquiry_id + "','" + date + "','" + customerid + "','" + Indentor_Name.Text + "','" + Indentor_Email.Text + "','" + selectedValue + "'," + quantity + "," + TotalProductValue + "," + TotalEnquiryValue + ",'" + "Open" + "','" + "Not Done" + "'," + Emp_Id + "," + Com_Id + ",'" + "MAIN" + "','" + date + "')";
                                cmd.ExecuteNonQuery();
                                conn.Close();
                            ...
                            else
                            {
                                ...
                                DateTime dt = DateTime.Now;
                                var date = dt.Date;
                                cmd.CommandText = "Insert into Enquiry Values('" + enquiry_id + "','" + date + "','" + customerid + "','" + Indentor_Name.Text + "','" + Indentor_Email.Text + "','" + selectedValue + "'," + quantity + "," + TotalProductValue + "," + 0 + ",'" + "Open" + "','" + "Not Done" + "'," + Emp_Id + "," + Com_Id + ",'" + "PART" + "','" + date + "')";
                                cmd.ExecuteNonQuery();
                                conn.Close();
